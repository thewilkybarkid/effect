      ...
      return core.exitSucceed(core.blocked(
        ...
        ensuring(
          core.withFiberRuntime((newFiber) => {
            ...
            newFiber.setFiberRefs(refs)
            newFiber._runtimeFlags = flags
            return op.i1
          }),
          core.withFiberRuntime((newFiber) => {
            newFiber.setFiberRefs(snap.refs)
            newFiber._runtimeFlags = snap.flags
            return core.unit
          })
        )
      ) as core.Blocked<any, any>)
    }
  ...
  core.uninterruptibleMask((restore) =>
    core.transplant((graft) =>
      core.withFiberRuntime((parent) => {
        ...
        const fibers = new Set<FiberRuntime<never, Exit.Exit<E, _> | Effect.Blocked<R, E, _>>>()
        ...
        const startOrder = new Array<FiberRuntime<never, Exit.Exit<E, _> | Effect.Blocked<R, E, _>>>()
        const joinOrder = new Array<FiberRuntime<never, Exit.Exit<E, _> | Effect.Blocked<R, E, _>>>()
        const residual = new Array<core.Blocked>()
        const collectExits = () => {
          const exits: Array<Exit.Exit<E, any>> = []
          for (let i = 0; i < startOrder.length; i++) {
            const f = startOrder[i]!
            const exit = f.unsafePoll()
            if (exit) {
              if (exit._op === "Success" && exit.i0._op === "Failure") {
                exits.push(exit.i0 as Exit.Exit<E, _>)
                continue
              }
            }
            if (f.isInterrupted()) {
              exits.push(core.exitFailCause(f.getInterruptedCause()))
            }
          }
          if (exits.length === 0) {
        ...
        const runFiber = <R, E, A>(eff: Effect.Effect<R, E, A>) => {
          const runnable = core.interruptible(graft(eff))
        ...
        const processingFiber = runFiber(
          core.async<any, any, any>((resume) => {
            const next = () => {
              if (todos.length > 0) {
                const a = todos.pop()!
                const todo = (batching ? core.step : core.exit)(restore(f(a, counter++)))
                ...
                fiber.addObserver((wrapped) => {
                  ...
                  fibers.delete(fiber)
                  const exit = (wrapped as Exit.Success<any, any>).i0 as Exit.Exit<any, any> | core.Blocked
                  if (exit._op === "Blocked") {
                    residual.push(exit)
                  } else {
                    results.push(exit)
                    if (exit._op === "Failure" && !interrupted) {
                      onInterruptSignal()
                    }
                  }
        ...
        return core.tap(
          core.asUnit(core.flatten(core.onInterrupt(
